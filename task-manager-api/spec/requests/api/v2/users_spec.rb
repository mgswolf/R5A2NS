...
RSpec.describe "Users API", type: :request do
  ...
  describe 'POST /auth' do
    ...
    context 'when the request params are valid' do
      ...
      it 'returns json data for the created user' do
        expect(json_body[:data][:email]).to eql(user_params[:email])
      end
    end

    context 'when the request params ar not valid' do
  ...
  describe 'PUT /auth' do
    ...
    context 'when the request params are valid' do
      ...
      it 'returns the json data fot the updated user' do
        expect(json_body[:data][:email]).to eq('new@email.com')
      end
    end

    context 'when the request params are invalid' do
      let(:user_params) { attributes_for(:user, email: 'infalid_email@') }
