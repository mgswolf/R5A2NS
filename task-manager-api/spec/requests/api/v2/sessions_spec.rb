...
RSpec.describe 'Sessions API', type: :request do
  ...
  describe 'POST /auth/sign_in' do
    ...
    context 'when the credential are correct' do
      ...
      it "returns the auth data in the headers" do
    ...
    context 'when the json data for the user with auth token' do
  ...
  describe 'DELETE /auth/sign_out' do
    ...
    it "changes the user auth token" do
      user.reload
      expect(user.valid_token?(auth_data['access-token'], auth_data['access-token'])).to be_falsey
    end
  end
end
